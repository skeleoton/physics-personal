...
initial_S = 10000
xsS=2.8979 # signal cross section (SM)
signal_SM_file = './rootdata/HW-7_SM_var.smear.root' 
...
# main training functuion (June 12, 2025: in progress!)
def train_xgboost(savetraining=False): 
    ...
    return model, initial_NB, Sweight, Bweight, X_test, y_test

# apply the given model
def apply_xgboost(model, initial_NB, Sweight, Bweight, X_test, y_test):
    #print(model)
    ...
    print('Initial signal cross section=', Sweight/Lumi)
    ...
    print('-')
    print('Final signal cross section=', Sweight/Lumi*eff_S)
...
# train the model:
trained_model, initial_NB, Sweight, Bweight, X_test, y_test = train_xgboost(savetraining=False)
# save the model:
save_model(trained_model, 'trained_model.pkl')
# laod the model: 
trained_model_test = load_model('trained_model.pkl')
# apply the model:
apply_xgboost(trained_model_test, initial_NB, Sweight, Bweight, X_test, y_test)
